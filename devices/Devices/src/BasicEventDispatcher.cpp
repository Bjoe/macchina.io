...
BasicEventDispatcher::BasicEventDispatcher(BasicRemoteObject* pRemoteObject, const std::string& protocol):
	Poco::RemotingNG::EventDispatcher(protocol),
	_pRemoteObject(pRemoteObject)
{
	_pRemoteObject->stateChanged += Poco::delegate(this, &BasicEventDispatcher::event__stateChanged);
}


BasicEventDispatcher::~BasicEventDispatcher()
{
	try
	{
		_pRemoteObject->stateChanged -= Poco::delegate(this, &BasicEventDispatcher::event__stateChanged);
...
void BasicEventDispatcher::event__stateChanged(const void* pSender, const int& data)
{
	if (pSender)
	{
		Poco::Clock now;
...
void BasicEventDispatcher::event__stateChangedImpl(const std::string& subscriberURI, const int& data)
{
...
const std::string BasicEventDispatcher::DEFAULT_NS("");
} // namespace Devices
} // namespace IoT
